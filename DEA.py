...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 05:08:45 2023
...
def VRS(DMU, X, Y, orientation, dual):
    ...
    if(orientation=="input" and dual==False):
        ...
        for r in DMU:
            try:
                ...
                #E[r]="The efficiency of DMU %s:%0.3f"%(r,m.objVal)
                E=m.objVal
                dmu_ke_vrs_io.append(r)
                efisiensi_vrs_io.append(E)
    ...
    #                print(u0_v[r])
            	...
            	print ('GurobiError reported')
        df_hasil_vrs_io=pd.DataFrame({'dmu':dmu_ke_vrs_io, 'efisiensi':efisiensi_vrs_io})
        return df_hasil_vrs_io
...
#                    print(v0_v[r])
        ...
        df_hasil=pd.DataFrame({'dmu':dmu_ke, 'efisiensi':efisiensi})
        return df_hasil
...
#DMU,X,Y=csv2dict("data.csv",in_range=[2,4],out_range=[5,8],assign=True)

"""
DMU, X=csv2dict_sep("input.csv")
DMU, Y=csv2dict_sep("output.csv")

print('---------------vrs output oriented-----------')
VRS(DMU, X, Y, orientation="output", dual=False)

print('------------input orientied-------------')
VRS(DMU, X, Y, orientation="input", dual=False)


print('-------vrs_0_dual---')
VRS(DMU, X, Y, orientation="output", dual=True)
"""
